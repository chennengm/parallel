...
int main()
{
    ...
    cout << "普通串行算法" << (tail - head) * 1000.0 / freq << "ms" << endl;
    ...
    QueryPerformanceCounter((LARGE_INTEGER*)&tail);
    cout << "SSE不对齐" << (tail - head) * 1000.0 / freq << "ms" << endl;

    initA2();
    QueryPerformanceCounter((LARGE_INTEGER*)&head);
    SIMD_AVX_gausseliminate(A2);
    QueryPerformanceCounter((LARGE_INTEGER*)&tail);
    cout << "AVX对齐" << (tail - head) * 1000.0 / freq << "ms" << endl;

    init(A);
    QueryPerformanceCounter((LARGE_INTEGER*)&head);
    SIMD_AVX_notaligned_gausseliminate(A);
    QueryPerformanceCounter((LARGE_INTEGER*)&tail);
    cout << "AVX不对齐" << (tail - head) * 1000.0 / freq << "ms" << endl;
}
